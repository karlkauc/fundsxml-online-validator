...
import { EVENT_OPEN_FEEDBACK, type FeedbackContext } from "../lib/links";

type Status = { kind: "idle" } | { kind: "sending" } | { kind: "sent" } | { kind: "error"; message: string };

/**
 * Modal feedback form. Opened from anywhere via `openFeedback()` (lib/links) —
 * the same window-event mechanism the XSD viewer uses.
 */
export function FeedbackDialog() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(EVENT_OPEN_FEEDBACK, onOpen);
    return () => window.removeEventListener(EVENT_OPEN_FEEDBACK, onOpen);
  }, []);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      await sendFeedback({
        ...
        xml_name: xmlDoc?.filename,
        xsd_name: xsdInfo?.main_filename,
  ...
  }, [message, email, website, context, xmlDoc, xsdInfo]);
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Bug, missing feature, a file that does not load — anything helps.
            </p>
          </div>
          ...
          >
            ...
            <div className="mt-3 flex items-center justify-between gap-3">
              <p className="text-[11px] text-slate-500 dark:text-slate-400">
                Stored without your IP address; file contents are never sent.
              </p>
