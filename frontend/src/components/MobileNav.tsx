// Mirrors the store's `viewMode` plus the validation panel, which on phones is
// a pane of its own (see App.tsx `validationOpen`).
export type MobilePane = "tree" | "diagram" | "validation";

const PANES: { pane: MobilePane; label: string; icon: string }[] = [
  { pane: "tree", label: "Tree", icon: "🌳" },
  { pane: "diagram", label: "Diagram", icon: "🕸" },
  { pane: "validation", label: "Validation", icon: "✅" },
];
